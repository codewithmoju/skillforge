...
export async function POST(request: NextRequest) {
  try {
    const { topic, areaId, existingTopics } = await request.json();
    ...
    // Construct context from existing topics if available
    const structureContext = existingTopics
      ? `
      STRICT CONSTRAINT: You must generate details for the following EXISTING TOPICS in this exact order. 
      Do NOT add new topics. Do NOT remove topics. Do NOT rename topics.
    ...
    
    {
      ...
      "topics": [
        {
          "id": "topic_1",
          "name": "Topic Name (MUST MATCH EXISTING IF PROVIDED)",
